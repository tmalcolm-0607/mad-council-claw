...
if (-not $ProjectRoot -and -not $PSBoundParameters.ContainsKey('ProjectRoot')) {
    # Walk up from script dir (.mad/scripts/) to find the repo root containing .claude/
...
"@
        ...
        "mutation-detection" {
            return @"
Review this LENS-CMS service codebase for bugs, security issues, performance problems, and convention violations. The project is a 5-layer .NET service (Common, DataAccess, BusinessLogic, DependencyInjection, API) that manages legal cases using Azure Cosmos DB.
