# Security Review Checklist — Recurring Pitfalls (S1-S15)

> **Source.** Adopted from the `lens-multi-model-review` plugin by Shayon Gupta (LENS-Common PR #5138039, v1.1.0). Folded in here so our `pr-review` skill carries the same S1-S15 taxonomy with the same severity rubric and "do not invent findings" discipline.

This checklist is **mandatory context for every `pr-review`**. It encodes recurring security pitfall *themes* that ship to production again and again across enterprise web services: misconfigurations that survive review, defense-in-depth layers that fall open under empty configuration, and code-level patterns that look benign but leak secrets, identities, or capability.
