...
|---|---|---|---|---|
| D-24 | Drop "all 7 phases in v1" target | (a) freeze foundation around M0..M11 + packaging minimum; M12..M14 + M18 marketplace post-foundation; (b) keep 7-phase target unchanged; (c) hybrid — ship 7-phase scaffolds RED but only M0..M11 GREEN | MEDIUM | Per design review M-I: 7-phase target is unsound for foundation wave. Wave-3 cross-model verification needed before HIGH promotion. |
| D-25 | M2 rename to "Governance kernel" (label-only) | (a) rename M2 = Governance kernel; (b) keep "Governance triad" label | MEDIUM | Per design review m4: "triad" undersells actual scope (audit + halt + cost + kill + degradation + quota + redaction + policy = 8 surfaces, not 3). Cosmetic but informative. |
| D-26 | Soul boundary enforcement mechanism | (a) compile-time package-boundary enforcement (separate modules + lint import-ban + test fixture attempting forbidden imports) + runtime check as defense-in-depth; (b) runtime check only (current FR-SOUL-001); (c) capability-based (object-capability model) | MEDIUM | Per design review M-J: runtime-only is too weak if privileged APIs remain importable. Wave-3 should evaluate (a) vs (c) — TS module boundaries are easier in v1, capability model is harder but more robust. |
