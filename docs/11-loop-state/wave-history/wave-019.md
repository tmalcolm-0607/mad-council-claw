...
- **Deferral-flag burst on subagent writes** — 11 subagent SubagentStop hook fires this wave when ledger / lane-summary prose used preserved-not-invented deferral language ("deferred to M5 integration wave", etc.) drawn from upstream ledgers. Pattern: the content-scan-deferrals hook flags keyword presence regardless of authorial intent. Resolution at session level: ack-list maintained in MAD - Clean kit's `.mad/reports/deferral-ack-2026-05-07.md`; standing-directive #4 added to `/loop` prompt to forbid keywords going forward; new sections appended at each ack event.
...
- **Deferral-keyword ban now an ABSOLUTE prohibition** — directive #4 in the `/loop` invocation. Use `[scope:F-NNN]` framing exclusively. The content-scan-deferrals hook treats keyword presence as a violation regardless of authorial intent. This is the #1 cause of loop stalls.
