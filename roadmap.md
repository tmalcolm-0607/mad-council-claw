...
|---|---|---|---:|---:|---:|---:|---:|---:|
| M0 | Project bootstrap | F-001..F-008 | 8 | 0 | 0 | 8 | 0 | 0 |
| M1 | Pluggable backend | F-009..F-013 | 5 | 0 | 4 | 1 | 0 | 0 |
...
| **TOTAL** | (121 base + 5 new = 126 active) + 18 deferred | | **144** | **104** | **4** | **18** | **18** | **0** |
...
| F-010 | anthropic-sdk-provider | 🟢 GREEN | `tests/unit/F-010-anthropic-backend.test.ts` (7/7 PASS) — wave-015 / lane-b; AnthropicBackend implements F-009 IBackendProvider with origin='anthropic'; deterministic STUB body — real `@anthropic-ai/sdk` deferred per `no-silent-deferrals.md` (gated on F-070 secure-storage + recorded-fixture harness); halt-semantics divergence from StubBackend documented (keeps session registered, yields finish/error per F-018 RUN_HALTED observability) |
| F-011 | copilot-sdk-provider | 🟢 GREEN | `tests/unit/F-011-copilot-backend.test.ts` (7/7 PASS) — wave-015 / lane-c; v1 deterministic stub (real Copilot CLI/SDK invocation deferred per ledger out-of-scope-notes — gated on CLI install + device-flow OAuth + recorded-fixture harness); halt-flips-state semantics matching F-010; default model 'gpt-5' constructor-injectable |
| F-012 | backend-factory | 🟢 GREEN | `tests/unit/F-012-backend-factory.test.ts` (6/6 PASS) — wave-015 / lane-d; `createBackend({kind, model})` dispatches to AnthropicBackend / CopilotBackend / StubBackend by string-literal kind; pure-function shape (no I/O); `BackendNotRegistered` realized via TS exhaustive-switch never-arm + runtime throw. Scope simplified vs ledger (createBackendProvider + MAD_BACKEND env override) — env resolution moved to caller per F-067 settings layer (M8) |
| F-013 | event-normalization | 🟢 GREEN | `tests/unit/F-013-backend-event-normalization.test.ts` (9/9 PASS) — wave-015 / lane-d; `BackendEvent` union from F-009 satisfies the cross-SDK normalization contract; F-013 contributes the convenience layer (4 type guards + `eventTextContent`) so downstream consumers don't re-implement narrowing. 5 ledger variants (`message_start` / `tool_use_input_delta` / `usage` / `cancelled` / `error`) tracked for future event-richness wave per `no-silent-deferrals.md` |
