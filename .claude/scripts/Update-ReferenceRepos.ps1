...
.DESCRIPTION
    This script manages reference repositories in the references directory.
    It can clone repos that don't exist, convert downloaded archives to proper
    git repos, and pull latest changes for existing repos.

    NOTE: The Azure DevOps organization, project, and $RepoMapping are examples.
    Edit $AzureDevOpsOrg, $AzureDevOpsProject, and $RepoMapping for your own
    set of reference repos.
...
.PARAMETER CleanNames
    ...
    For example: "sample-repo (1)" -> "sample-repo"
...
if (-not $ReferencesPath) {
    if ($PSScriptRoot) {
        ...
        $ReferencesPath = Join-Path (Get-Location) "references"
    }
}
...
# Azure DevOps organization and project -- LENS-DCS standardization loop
...
# Three repos required for the LENS-DCS standardization loop:
#   LENS-DCS    -- target service being migrated to layered architecture
#   LENS-Docs   -- source-of-truth for `lens-standards-audit:standards-audit`
#   LENS-Common -- source for `lens-aspnet-structure` + `lens-pipeline-audit` plugins
$RepoMapping = @{
    "LENS-DCS"    = "LENS-DCS"
    "LENS-Docs"   = "LENS-Docs"
    "LENS-Common" = "LENS-Common"
}
...
if ($RepoMapping.Count -eq 0) {
    Write-Status "RepoMapping is empty. Edit this script and add entries to `$RepoMapping before running." -Type Warning
    exit 0
}
