...
| RG-13 | Wave-002 Lane C Copilot CLI dispatch had Opus timeout at 600s; cross-model agreement table is empty. Single-model (gpt-5.5) review captured 5 Critical + 12 Major + 4 Minor + 5 Praise. Cross-model both-flag-CRITICAL hard-block rule could not fire. | wave-002 / lane-c | 2026-05-07 | HIGH | wave-3 should re-dispatch with `-TimeoutSeconds 1200` OR use synchronous role-split second voice for Opus path; verify cross-model agreement on F-127..F-134 promotions. |
| RG-14 | OpenAI A2A protocol cross-org auth model evolving toward OAuth 2.1 + TLS pinning per MCP roadmap 2026; F-122 a2a-endpoint-exposure does not yet specify auth surface. | wave-002 / lane-c design review C-3, M-H | 2026-05-07 | HIGH | wave-3 architecture lane: produce A2A external-endpoint security gate sub-spec; defer F-122 external HTTPS until decision. |
| RG-15 | Anthropic + Copilot SDK event-shape divergence not characterized; `IBackendProvider` over-normalization risk per design review M-A. Need empirical event-shape diff across both SDKs. | wave-002 / lane-c design review M-A | 2026-05-07 | MEDIUM | wave-3 architecture lane: enumerate Anthropic SDK event types vs Copilot SDK event types; design canonical envelope preserving raw provider payload. |
