...
.DESCRIPTION
    ...
    Supports batch processing, dynamic PR discovery via --recent, and multi-repo scanning via --all-repos.

.PARAMETER PRIds
    Array of PR IDs to process. Ignored when -Recent is specified.

.PARAMETER Recent
    Fetch N most recent completed PRs from the target repository (or each repo if -AllRepos).

.PARAMETER AllRepos
    Iterate all configured repositories. Applies -Recent per repo (default 3 per repo).

.PARAMETER ReposToScan
    Subset of repos to scan when -AllRepos is used. Edit the default list to match
    your ecosystem.
...
.PARAMETER Repository
    Repository name. Default: consumer-project
...
.EXAMPLE
    .\Mine-PRPatterns.ps1 -Recent 5 -Repository "consumer-project"
    # Fetches 5 most recent completed PRs from consumer-project

.EXAMPLE
    .\Mine-PRPatterns.ps1 -AllRepos -Recent 3
    # Fetches 3 most recent PRs from each configured repo (capped at -MaxPRs)

.EXAMPLE
    .\Mine-PRPatterns.ps1 -AllRepos -ReposToScan @("consumer-project", "another-service")
...
param(
    ...
    [Parameter()]
    [string[]]$ReposToScan = @(
        # Edit this list to match your ecosystem
        "consumer-project"
    ),
    ...
    [Parameter()]
    [string]$Repository = "consumer-project",
...
if ($AllRepos) {
    # -AllRepos mode: iterate repos, fetch recent PRs from each
    $recentPerRepo = if ($Recent -gt 0) { $Recent } else { 3 }
    Write-Host "`nScanning $($ReposToScan.Count) repositories ($recentPerRepo recent PRs each)..." -ForegroundColor Cyan
