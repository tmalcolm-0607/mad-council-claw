...
.PARAMETER SolutionPath
    Path to .sln file (auto-discovers consumer-project if not specified)
...
.PARAMETER SkipBicepLint
    Skip the Bicep lint gate (Gate 5)

.PARAMETER GitDiffBase
    Base branch/ref for detecting changed files (used by Bicep lint gate).
    Defaults to origin/main.

.EXAMPLE
    .\Run-DotnetGates.ps1
    .\Run-DotnetGates.ps1 -SolutionPath C:\source\consumer-project\src\CMS.sln
...
param(
    ...
    [switch]$SkipBicepLint,

    [string]$GitDiffBase = "origin/main",

    [string]$BicepExePath = "C:/Users/tonym/.azure/bin/bicep.exe"
)
...
if (-not $SolutionPath) {
    $candidates = @(
        "C:\source\consumer-project\sources\dev\CMS\src\CMS.sln",
        "C:\source\consumer-project\src\CMS.sln",
        "C:\source\consumer-project\src\CMS.sln"
    )
...
# --- Gate 1: Build ---
# NOTE: This gate builds CMS.sln which may fail locally with NU1900 (Worker/Worker.Tests).
# CLAUDE.md recommends building the 4 test projects individually instead.
# See "Full Gate Suite" in CLAUDE.md for the correct per-project build commands.
if (-not $SkipBuild) {
...
# --- Gate 5: Bicep Lint ---
if (-not $SkipBicepLint) {
    Write-Host ""
    Write-Host "--- Gate 5: Bicep Lint ---" -ForegroundColor Yellow

    if (-not (Test-Path $BicepExePath)) {
        Write-Host "  [WARN] bicep.exe not found at $BicepExePath - skipping Bicep lint gate" -ForegroundColor DarkYellow
        $gateResults += "Bicep Lint: SKIPPED (bicep.exe not found)"
    } else {

    # Find changed .bicep and .bicepparam files relative to the diff base
    $ErrorActionPreference = "Continue"
    $changedBicepRaw = git diff --name-only --diff-filter=ACMR "$GitDiffBase" -- '*.bicep' '*.bicepparam' 2>&1
    $ErrorActionPreference = "Stop"

    # Filter to actual file paths (ignore git errors like "unknown revision")
    $changedBicep = @()
    if ($LASTEXITCODE -eq 0 -and $changedBicepRaw) {
        $changedBicep = @($changedBicepRaw | Where-Object { $_ -match '\.(bicep|bicepparam)$' })
    }

    if ($changedBicep.Count -eq 0) {
        Write-Host "  No .bicep/.bicepparam changes - skipping" -ForegroundColor DarkYellow
        $gateResults += "Bicep Lint: SKIPPED (no changes)"
    } else {
        Write-Host "  Found $($changedBicep.Count) changed bicep file(s)" -ForegroundColor Cyan
        $bicepFailed = 0
        $bicepPassed = 0

        # Resolve repo root once outside the loop
        $repoRoot = git rev-parse --show-toplevel 2>$null

        foreach ($file in $changedBicep) {
            ...
            if (-not (Test-Path $absPath)) {
                Write-Host "    SKIP: $file (file not found on disk)" -ForegroundColor DarkYellow
                continue
            }

            $ErrorActionPreference = "Continue"
            $lintOutput = & "$BicepExePath" lint "$absPath" 2>&1 | Out-String
            ...
            if ($lintExitCode -eq 0) {
                ...
                $bicepFailed++
                # Show lint errors (first 5 lines)
                $lintOutput -split "`n" | Where-Object { $_.Trim() } | Select-Object -First 5 | ForEach-Object {
                    Write-Host "      $($_.Trim())" -ForegroundColor DarkYellow
                }
            }
        }
        ...
        if ($bicepFailed -eq 0) {
            Write-Host "  PASS: All $bicepPassed bicep file(s) lint clean" -ForegroundColor Green
            $gateResults += "Bicep Lint: PASS ($bicepPassed file(s))"
        } else {
            Write-Host "  FAIL: $bicepFailed of $($bicepPassed + $bicepFailed) file(s) have lint errors" -ForegroundColor Red
            $gateResults += "Bicep Lint: FAIL ($bicepFailed file(s) failed)"
            $failedGates++
        }
    }
    } # end Test-Path else
    ...
    Write-Host ""
    Write-Host "--- Gate 5: Bicep Lint (SKIPPED) ---" -ForegroundColor DarkYellow
    $gateResults += "Bicep Lint: SKIPPED"
}
