...
| ID | Decision | Options | Source | Date opened | Confidence | Suggested closure path |
|---|---|---|---|---|---|---|
| ~~D-35~~ **RESOLVED 2026-05-07 (wave-017 / lane-a)** | F-138 engine-cycle-orchestrator scope | **Closed: option (a) full composition layer landed.** F-138 GREEN at `packages/engine-core/src/cycle.ts`; ledger at `docs/03-feature-catalog/M0-bootstrap/F-138-engine-cycle-orchestrator.md`; test at `tests/unit/F-138-engine-cycle-orchestrator.test.ts` (3/3 PASS). v1 single-iteration; F-017 redaction at audit-egress, F-021 ladder, F-020 polling, F-022 per-spawn, F-013 usage variant, multi-iteration loop deferred per `no-silent-deferrals.md` (7 honest scope-narrowing notes in ledger §out-of-scope-notes). Wave-016 HARD-BLOCK F1 RESOLVED at the composition-spine level. | wave-016 / lane-d C-1 (Opus) + cycle-orchestration-not-wired (gpt-5.5) | 2026-05-07 | **HIGH** (both models agree) | **Resolved 2026-05-07 by wave-017 / lane-a** |
| ~~D-36~~ **RESOLVED 2026-05-07 (wave-019 / lane-b)** | F-139 backend-event-usage-variant addition timing | **Closed: option (a) — added `usage` variant to BackendEvent now.** F-139 GREEN at `packages/engine-core/src/backend-event-variant.ts`; ledger at `docs/03-feature-catalog/M1-backend/F-139-backend-event-usage-variant.md`; test at `tests/unit/F-139-backend-event-usage-variant.test.ts` (6/6 PASS). BackendEvent gains 5th variant `{type:'usage', input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, model, backend?}` in backend.ts; isUsageEvent + eventTextContent extension in backend-events.ts; usageEventToCostEntry mapper produces F-019 CostEntryInput from a usage event + correlation. Caller-deferred pricing (no PriceLookup -> usd_estimate=0); F-019's observable-only contract preserved. F-138 cycle.ts can now compose this primitive in a future iteration to populate cost-ledger rows from BackendEvent. F-131 fanout-budget-governor unblocked. | wave-016 / lane-d C-2 (Opus only) + cost.ts:6 contract claim | 2026-05-07 | **HIGH** (resolved per option (a) default recommendation) | **Resolved 2026-05-07 by wave-019 / lane-b** |
